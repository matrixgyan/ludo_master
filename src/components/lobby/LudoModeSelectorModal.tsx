...
interface LudoModeSelectorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectMode: (mode: PlayerModeOption, entryFee: number, prizePool: number) => void;
  balance: number;
}
...
export const LudoModeSelectorModal: React.FC<LudoModeSelectorModalProps> = ({
  ...
  const handleStartMatching = () => {
    ...
    onSelectMode(selectedMode, selectedFee, currentPrize);
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md select-none">
        <motion.div
          ...
          className="relative w-full max-w-md bg-gradient-to-b from-[#1b0833] via-[#120524] to-[#0a0215] rounded-3xl border-2 border-violet-500/40 shadow-[0_25px_60px_rgba(0,0,0,0.95)] overflow-hidden text-white flex flex-col"
        >
          {/* Ambient Top Glow */}
          <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-72 h-36 bg-violet-600/30 rounded-full blur-3xl pointer-events-none" />

          {/* Top Header */}
          <div className="relative px-5 py-3.5 border-b border-violet-800/40 flex items-center justify-between z-10 bg-black/25">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-amber-500 to-yellow-300 p-0.5 shadow-[0_0_12px_rgba(251,191,36,0.5)] flex items-center justify-center">
                <div className="w-full h-full bg-[#1b0833] rounded-[10px] flex items-center justify-center">
                  <Crown className="w-4 h-4 text-amber-300 fill-amber-300" />
                </div>
              </div>
              <div>
                <h3 className="text-base font-black tracking-tight text-white flex items-center gap-1.5 leading-tight">
                  <span>Ludo Online Arena</span>
          ...
          <div className="p-4 space-y-4 z-10">
            {/* 3D ILLUSTRATED GAME-MODE TILES (2 PLAYERS, 3 PLAYERS, 4 PLAYERS) */}
            ...
            {/* ENTRY FEE SELECTION */}
            <div className="pt-2 border-t border-violet-800/40 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-extrabold uppercase text-violet-300/90 tracking-wide">
                  Entry Fee
                </span>
          ...
          <div className="p-4 border-t border-violet-800/40 bg-[#0d021c] z-10">
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={handleStartMatching}
              className="w-full py-3 px-5 rounded-2xl font-black text-sm bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-slate-950 shadow-[0_4px_25px_rgba(251,191,36,0.5)] border border-yellow-200 flex items-center justify-center gap-2 transition-all cursor-pointer uppercase tracking-wider"
            >
              <Zap className="w-4 h-4 text-slate-950 fill-slate-950" />
              <span>ENTER {selectedMode} PLAYER BATTLE</span>
            </motion.button>
          </div>
        </motion.div>
      </div>
