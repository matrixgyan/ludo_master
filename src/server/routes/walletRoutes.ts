...
/**
 * POST /api/wallet/match-win
 * Instantly credits match prize winnings to the authenticated player's wallet & ledger
 */
const MatchWinSchema = z.object({
...
walletRouter.post('/api/wallet/match-win', async (req: Request, res: Response) => {
  try {
    const headerOrAuthUserId = resolveUserId(req);
    const body = MatchWinSchema.parse(req.body);
    const userId = body.userId || headerOrAuthUserId;

    const numPrize = typeof body.prizePool === 'string' ? parseFloat(body.prizePool) : body.prizePool;
    if (isNaN(numPrize) || numPrize <= 0) {
      res.status(400).json({ success: false, error: 'Invalid prizePool amount' });
      return;
    }

    const cleanMatchId = body.matchId || `match_${Date.now()}`;
    const idempotencyKey = `match_win_${cleanMatchId}_${userId}`;

    // Atomically credit winner's available balance in ledger & database
    const creditResult = await LedgerService.creditDeposit(
      ...
      idempotencyKey,
      {
        ...
        gameType: body.gameType || 'supreme',
    ...
    // Fetch refreshed wallet
    const updatedWallet = await LedgerService.getUserWallet(userId);

    Logger.info(`[MATCH WINNER PAYOUT] Credited ₹/${numPrize} prize money to user ${userId} for match ${cleanMatchId}`);

    res.json({
      success: true,
      message: `Successfully credited prize of ${numPrize} to user ${userId}`,
      transactionId: creditResult.transactionId,
      newAvailableBalance: creditResult.newAvailableBalance,
      wallet: updatedWallet,
    });
  } catch (err: any) {
    Logger.error('Match win payout error', err);
    res.status(500).json({ success: false, error: err?.message || 'Failed to credit match winnings' });
  }
});

/**
 * POST /api/wallet/match-entry
 * Deducts / locks match entry fee for starting real matches
 */
const MatchEntrySchema = z.object({
  userId: z.string().optional(),
  matchId: z.string().optional(),
  entryFee: z.union([z.number(), z.string()]),
  gameType: z.string().optional(),
});

walletRouter.post('/api/wallet/match-entry', async (req: Request, res: Response) => {
  try {
    const headerOrAuthUserId = resolveUserId(req);
    const body = MatchEntrySchema.parse(req.body);
    const userId = body.userId || headerOrAuthUserId;

    const numFee = typeof body.entryFee === 'string' ? parseFloat(body.entryFee) : body.entryFee;
    if (isNaN(numFee) || numFee <= 0) {
      res.json({ success: true, message: 'Free match entry' });
      return;
    }

    const cleanMatchId = body.matchId || `match_${Date.now()}`;
    const idempotencyKey = `match_entry_${cleanMatchId}_${userId}`;

    // Verify balance & settle entry fee
    const wallet = await LedgerService.getUserWallet(userId);
    if (parseFloat(wallet.availableBalance) >= numFee) {
      await LedgerService.lockFundsForWithdrawal(userId, numFee.toFixed(8), idempotencyKey);
      await LedgerService.settleWithdrawal(userId, numFee.toFixed(8), '0.00000000', `${idempotencyKey}_settle`);
    }

    const updatedWallet = await LedgerService.getUserWallet(userId);
    Logger.info(`[MATCH ENTRY FEE] Deducted ${numFee} entry fee for user ${userId} in match ${cleanMatchId}`);
    ...
    Logger.warn('Match entry fee deduction notice', { error: err?.message });
    res.json({ success: false, error: err?.message || 'Insufficient balance for entry fee' });
  }
});
