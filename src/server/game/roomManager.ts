...
export class RoomManager {
  ...
  public static async ensureMatchPoolsSeeded(): Promise<void> {
    ...
    try {
      ...
      Logger.debug('Database pool seeding notice (using in-memory fallback)', err);
    }
  }
  ...
  public static async replenishJoinableRooms(): Promise<void> {
    for (const poolDef of ALL_MATCH_POOLS) {
      ...
      try {
        ...
        Logger.debug(`Replenish note for pool ${poolDef.poolKey}`, err);
      }
    }
  }
        ...
        if (isPostgresConfigured()) {
          ...
          if (pool) {
            try {
              const client = await pool.connect();
              try {
                // Ensure the parent match_pools record exists so foreign key constraints never fail
                await client.query(
                  `INSERT INTO match_pools (
                     id, pool_key, game_mode, player_count, entry_fee, rule_version,
                     platform_fee_rate, is_active, min_buffer_rooms
                  ...
                  [
                    ...
                    poolDef.entryFee,
                    poolDef.ruleVersion,
                    poolDef.platformFeeRate,
                    poolDef.isActive,
                    poolDef.minBufferRooms,
                  ]
                );
              ...
              Logger.debug(`Postgres room check notice for ${poolDef.poolKey}, using memory fallback`, pgErr);
            }
          }
        }
        ...
        try {
          ...
          try {
            ...
            return res.rows.map((row) => {
              ...
              const feeNum = parseFloat(row.entry_fee || '0');
          ...
          Logger.debug('PostgreSQL getJoinableRooms notice, using memory store', pgErr);
        }
      }
    }
